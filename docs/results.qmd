...
hpart <- hrounds %>%
  ...
  summarise(
    sum_pct_slack_claimed = sum(reported_amount - true_amount)/sum(6000 - true_amount),
    truthful = as.integer(sum_pct_slack_claimed == 0),
    .groups = "drop"
...
datasummary(
  (`% Truthful`= truthful) + 
    (`% Slack Claimed`=pct_slack_claimed) ~ experiment*(N + Mean + SD), 
  data = hrounds
)

chisq.test(hrounds$truthful, hrounds$experiment)
t.test(pct_slack_claimed ~ experiment, data = hrounds %>% filter(pct_slack_claimed > 0))
wilcox.test(pct_slack_claimed ~ experiment, data = hrounds %>% filter(pct_slack_claimed > 0))
...
datasummary(
  (`% Always Truthful`= truthful) + 
    (`% Total Slack Claimed`= sum_pct_slack_claimed) +
...
t.test(sum_pct_slack_claimed ~ experiment, data = hpart %>% filter(sum_pct_slack_claimed > 0))
wilcox.test(sum_pct_slack_claimed ~ experiment, data = hpart %>% filter(sum_pct_slack_claimed > 0))
```
...
## Figure 2: Average % Slack Claimed by Round and Condition
...
df <- hrounds %>%
  ...
  filter(!is.na(pct_slack_claimed)) %>%
  summarise(
    mn_slack = mean(pct_slack_claimed),
    lb = mn_slack - 1.96*sd(pct_slack_claimed)/sqrt(length(pct_slack_claimed)),
    ub = mn_slack + 1.96*sd(pct_slack_claimed)/sqrt(length(pct_slack_claimed))
  )

ggplot(df, aes(x = round, y = mn_slack, color = experiment)) +
  geom_point() +
...
```{r}
mod_payoff_part <- feols(payoff ~ experiment*role, data = gpart)
