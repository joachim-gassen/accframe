...
rounds <- read_csv("../data/generated/eaadc24ll_rounds.csv", show_col_types = FALSE) %>%
	mutate(
...
part <- read_csv("../data/generated/eaadc24ll_participants.csv", show_col_types = FALSE) %>%
  mutate(
...
\begin{center}
\includegraphics[height =0.8\textheight]{"materials/eaadc24_exp_inst.jpeg"}
\end{center}
...
plot_demog_histogram <- function(demog) {
  ...
  ggplot(df) +
    geom_bar(aes(x = {{demog}}), fill = col_trr266_iceblue, color = NA) +
    labs(x = "", y = "") +
