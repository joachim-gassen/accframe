...
message(sprintf("%% Truthful reporting: %.2f %%", 100*pct_true))
...
pwr.2p.test(h = es, n = 500)
pr <- pwr.2p.test(n = 500, power = 0.8)
...
ssize_lying = round((1 - pct_true)*500)
pwr.t.test(
...
pwr.t.test(50, (0.1*mn_pct_slack_claimed_part)/sd_pct_slack_claimed_part)
