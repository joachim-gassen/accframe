...
system_prompt = """
    ...
    3. How much does it reveal that the person cares about establishing and promoting trust?
...
def classify_response(statement, model = "gpt-4o", openai_api_key = None):
    ...
    while resp_dict is None:
        ...
        try:
            ...
            assert "trust" in resp_dict, "Bot's response does not contain 'trust'."
            ...
            assert 1 <= resp_dict["trust"] <= 10, "trust score is not between 1 and 10."
...
if __name__ == "__main__":
    df = pd.read_csv("data/generated/trust_2024-06-18_rounds.csv")
    for idx, row in df.iterrows():
        statement = row["sent_reason"]
        scores = classify_response(statement)
        df.loc[idx, "reason_sent_self_payoff"] = scores["self_payoff"]
        df.loc[idx, "reason_sent_other_payoff"] = scores["other_payoff"]
        df.loc[idx, "reason_sent_fairness"] = scores["fairness"]
        df.loc[idx, "reason_sent_trust"] = scores["trust"]
        statement = row["sent_back_reason"]
        scores = classify_response(statement)
        df.loc[idx, "reason_back_self_payoff"] = scores["self_payoff"]
        df.loc[idx, "reason_back_other_payoff"] = scores["other_payoff"]
